...
struct ExpandingHexagon: View {
    ...
    var body: some View {
        Polygon(sides: 6) // this slides decides, How many slides It would have, We can increase or decrease its value based on our need
            ...
            .frame(width: 90, height: 90) // we can update the frame width and height too
            .scaleEffect(scale)
            .animation(.easeInOut(duration: 1.0).repeatForever(autoreverses: true), value: scale) // duration can be updated as per the need of animation duration
            .onAppear { scale = 1.2 }
    }
}
